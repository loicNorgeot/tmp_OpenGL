...
void font::render(std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color){
  ...
  glUseProgram(shader->ID);
  glm::mat4 projection = glm::ortho(0.0f, static_cast<GLfloat>(parentWindow->scene->view->w), 0.0f, static_cast<GLfloat>(parentWindow->scene->view->h));
  send(shader->ID, projection, "projection");
